...
def register_docs_commands(group: click.Group) -> None:
    ...
    def export_command(
        ...
        if output_path:
            ...
            payload = {"document_id": document_id, "format": export_format, "path": str(target)}
            if use_json_output(ctx, json_output):
                print_json(payload)
            else:
                print_success(f"Exported to: {target}")
            return

        if export_format in {"txt", "html"}:
            text = data.decode("utf-8", errors="replace")
            payload = {"document_id": document_id, "format": export_format, "content": text}
            if use_json_output(ctx, json_output):
                print_json(payload)
            else:
                print_human(text)
